...
int RCseries() {
    ...
    wprintf(L"\nEnter a letter to exit (break the code)\n"
            "enter 0 in place of unknown\n");
    while(1){
    wprintf(L"Enter values for |   V   |   f   |   R   |   C   |   Xc   |   Z   |   I   |   S   |   Q   |   P   |   phi   (with spaces): \n"); 
    fgets(buffer, sizeof(buffer), stdin);
    Inputs result = RCsCalc(buffer);
    double  V = result.V, 
            f = result.f,       
            R = result.R, 
            C = result.C, 
    ...
    if (V == 0 && f == 0 && R == 0 && C == 0 && Xc == 0 && Z == 0 && I == 0 && P == 0 && phi == 0) {
        wprintf(L"Exiting...\n");
        return 0; 
    }
    int nozero = 0;
    ...
    if(R < 0 || C < 0 || Xc < 0 || Z < 0 || S < 0 || phi < 0 || f < 0) {
        wprintf(L"Only V and I can be negative\n");
        fflush(stdin);
        continue; 
        }
    
    solve_rc(&result, (Outputs *)&result);
